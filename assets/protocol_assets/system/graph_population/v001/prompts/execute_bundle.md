...
continue_until: <first_completed_action|current_loop_complete|stop_condition>
validation_mode: <bootstrap|strict>

Read the manifest, domain protocol, ordered loop specs, cursor, execution log,
and candidate graph JSON. Identify the next legal action. Execute only the
bounded scope requested by continue_until. Validate affected graph state, append
